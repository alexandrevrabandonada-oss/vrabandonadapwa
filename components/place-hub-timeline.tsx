...
import type { PlaceHubTimelineEntry } from "@/lib/territories/types";

type Props = {
  entries: PlaceHubTimelineEntry[];
};

export function PlaceHubTimeline({ entries }: Props) {
  return (
        ...
        entries.map((entry) => (
          <article className="theme-hub-timeline__entry" key={entry.id}>
            ...
            <div className="theme-hub-timeline__content">
              ...
              <h3>{entry.title}</h3>
              <p>{entry.excerpt || entry.timeline_note || "Marco territorial."}</p>
              {entry.href ? (
