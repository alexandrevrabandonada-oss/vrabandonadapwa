...
import { getSharePackFormatLabel, getSharePackStatusLabel, getSharePackStatusTone } from "@/lib/share-packs/navigation";
    ...
    <article className={`card share-pack-card ${compact ? "share-pack-card--compact" : ""}`.trim()}>
      ...
      <div className="share-pack-card__body">
        <div className="meta-row">
          ...
          <span>{getSharePackFormatLabel(pack.preferred_format)}</span>
        </div>
        <h3>{pack.title}</h3>
        <p>{pack.caption}</p>
